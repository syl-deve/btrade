...
- **`main.py`**: FastAPI 앱, 인증, API 라우트, 트레이딩 루프
  - `_run_db_migrations()`: 모듈 로드 시 즉시 실행. BUY fee = `total_amount × 0.0004`, SELL fee = `total_amount × 0.0008`
  - `_reset_position()`: avg_buy_price, highest_profit_rate, buy_count, position_opened_at 초기화
  - `_record_sell()`: 매도 기록. fee = 매수원금×0.04% + 매도금액×0.04% 합산. `_reset_position` 호출
...
|---|---|---|---|
| GET | `/api/status` | 세션 | 잔고, 지표, 거래내역, 설정, 통계 전체 |
...
- `hintData`에 7개 스탯 카드 + 11개 설정 항목 한/영 설명 포함
