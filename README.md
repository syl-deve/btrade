...
Current dashboard authentication uses a single administrator account:
...
- **Session cookie**: random server-side `SESSION_TOKEN`, stored as `HttpOnly`, `SameSite=Lax`; `Secure` is enabled on HTTPS requests
